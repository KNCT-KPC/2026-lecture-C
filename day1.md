...
まずはLinuxコマンドの練習も兼ねて、空のファイルを作ります。

```sh
touch hello.c
```
このコマンドで `hello.c` という空ファイルが作成されます。
できたら次に進みましょう。
</div>
<div>
...
保存してエディタを終了します。
- Vim: `Esc` を押して `:wq` と入力し `Enter`  
（これだけ覚えればOK！）
- Emacs: `Ctrl-x` のあとに `Ctrl-s` で保存  
`Ctrl-x` のあとに `Ctrl-c` で終了  
（ね、面倒でしょ？）
</div>
<div>
...
書いたプログラムを**コンパイル**（コンピュータが実行できる形式に変換）します。

```sh
gcc hello.c -o hello
```
成功すると何もメッセージは出ませんが、同じフォルダに `hello` というファイルが作成されます。
これが実行可能なプログラムです！
</div>
<div>
...
<div class="grid grid-cols-[3fr_2fr] gap-4 mt-4 text-left">
<div>

### コンパイルエラー
もし `gcc` コマンドでズラズラと英語が出たら、それは**コンパイルエラー**です。
**原因の99%はタイポ（打ち間違い）です。**
C言語は1文字違うだけでもコンパイルが通りません。
落ち着いてエラーメッセージを読み、ソースコードを見直しましょう。これが**デバッグ**です。

<br>

<div class="bg-blue-100 text-blue-900 p-4 rounded text-sm font-bold">
エラーは敵ではありません。<br>
「ここが間違ってるよ」と教えてくれる親友です！感謝しろ！
</div>
