...
### 課題1の解説
ターミナルに「自分の名前 年齢」を出力するプログラム  
そのまま `printf` で表示させればOKです。
...
### メモリの確保
- 変数を作ると、コンピューター上の  
「どこかのメモリ」を確保し、そこに値を入れます
- **メモリを事前に確保しないといけない関係上  
使う前に「宣言」しないといけません。**
...
- **型**は、その変数に  
「どういった種類の値が入るのか」を示します。
  - 整数、実数、文字列など…  
  事前にコンピューターに分からせる必要があります。
- イコール(`=`)は**代入**を意味します。
...
変数を宣言した後は  
**必ず初期化** をしなければなりません！  
初期化しないで使った場合、元々メモリが確保される前にあった得体の知れない値が使用されてしまいます
...
💡 <b>超重要！</b><br>
変数名は<br><b>「なんの値を持つのか分かりやすいように」</b><br>つけましょう！！！<br><br>
...
### 解説
1. `int age = 18;` で変数の宣言と代入を行う
2. `printf` の中に `%d` を書き  
文字列のあとにカンマ `,` を打ってから  
変数 `age` を渡しています。

これで変数に保存した値を自由に取り出して表示できるようになりました
...
といっても、そこまで難しい内容ではありません。普通に式を書けばOK
...
💡 <b>ポイント</b><br>
掛け算は <code>×</code> ではなく <code>*</code>（アスタリスク）<br>割り算は <code>÷</code> ではなく <code>/</code>（スラッシュ）を使います<br>
これ以外にも演算子は色々ありますが、今は上記だけで十分<br>
必要であれば調べてみましょう
</div>
...
次回は、プログラムに「判断」をさせる方法を学びます

- **前回課題の解説** (`scanf` の詳しい使い方)
- **条件分岐** (`if`文)
- **「もし～なら…する」** (プログラムの分かれ道)
